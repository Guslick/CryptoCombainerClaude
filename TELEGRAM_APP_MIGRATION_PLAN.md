...
## Browser chart status
- Implemented browser rendering in Mini App using Chart.js.
- Added endpoint `/api/chart/latest` that returns latest persisted `TradingState`.
- Telegram updates now have text fallback when screenshot is unavailable (no hard dependency on Xvfb).
