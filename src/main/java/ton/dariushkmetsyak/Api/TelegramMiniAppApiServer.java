...
public class TelegramMiniAppApiServer {
    ...
    public void start() throws IOException {
        ...
        server.createContext("/api/chart/latest", this::serveLatestChartData);
    ...
    private void serveLatestChartData(HttpExchange exchange) throws IOException {
        if (!"GET".equalsIgnoreCase(exchange.getRequestMethod())) {
            writeJson(exchange, 405, "{\"ok\":false,\"message\":\"Method not allowed\"}");
            return;
        }

        File stateDir = new File("trading_states");
        if (!stateDir.exists() || !stateDir.isDirectory()) {
            writeJson(exchange, 200, "{\"ok\":true,\"hasState\":false,\"priceHistory\":[]}");
            return;
        }

        File[] stateFiles = stateDir.listFiles((dir, name) -> name.endsWith(".json") && !name.contains(".backup."));
        if (stateFiles == null || stateFiles.length == 0) {
            writeJson(exchange, 200, "{\"ok\":true,\"hasState\":false,\"priceHistory\":[]}");
            return;
        }

        Arrays.sort(stateFiles, Comparator.comparingLong(File::lastModified).reversed());
        TradingState state = TradingState.loadFromFile(stateFiles[0].getPath());

        String response = mapper.writeValueAsString(state);
    ...
    private void serveMiniApp(HttpExchange exchange) throws IOException {
        ...
        byte[] bytes;
        try (InputStream in = TelegramMiniAppApiServer.class.getClassLoader().getResourceAsStream("miniapp/index.html")) {
            if (in == null) {
